...
# Get the bot token from Koyeb's environment variables
...
async def otc_response(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Simulate analysis (2-5 seconds delay)
    analyzing_message = await update.message.reply_text(
        f"📊 Analyzing Chart for *{user_choice}* ... Please wait.",
        parse_mode="Markdown"
    )
    await asyncio.sleep(random.uniform(2, 5))  # Random delay between 2 and 5 seconds
    ...
    try:
        ...
        # Restore the original keyboard with OTC pairs
        await update.message.reply_text(
            "You can now select a new pair!",
...
def main():
    if not TOKEN:
        print("Error: BOT_TOKEN is missing. Make sure it's set in Koyeb environment variables.")
        return
